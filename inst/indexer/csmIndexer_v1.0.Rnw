...
urlFun = function(x,length = 80){
  r = rep("",length(x))
  for(j in 1:length(x)){
    n = ceiling(nchar(x[j])/length)
    for(i in 1:n){
    r[j] = paste0(r[j], "\\url{",substr(x[j],(i-1)*length + 1,i*length),"}")
    if(i < n){
      r[j] = paste0(r[j]," \\\\ ")
    }
    }
